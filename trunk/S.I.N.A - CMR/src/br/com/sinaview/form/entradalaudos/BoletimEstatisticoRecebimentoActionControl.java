...
import br.com.sinacontrol.controller.BoletimEstatisticoRecebimentoController;
 ...
 * @author ritacosta
 */
public class BoletimEstatisticoRecebimentoActionControl implements ActionListener {

    private FomrBoletimRecebimentoEstatistico form;
    private Long idBoletim;
    private List<BoletimEstatisticoRecebimento> listBoletim;
    private List<Prestador> prestadores;
    private List<Procedimento> procedimentos;

    public BoletimEstatisticoRecebimentoActionControl(FomrBoletimRecebimentoEstatistico form) {
        this.form = form;
        addBtnToForm();
        refreshCombo();
        enableFilds(false);
    ...
    private void onSaveProducao() {
        BoletimEstatisticoRecebimento boletim = new BoletimEstatisticoRecebimento();
        ...
        int result = 0;
        if (idBoletim == null) {
            result = new BoletimEstatisticoRecebimentoController().save(boletim);
        } else {
            boletim.setId(idBoletim);
            result = new BoletimEstatisticoRecebimentoController().update(boletim);
            idBoletim = null;
            refreshTableGeral();
        }
        if (result == 1) {
    ...
    private void onAlterarProducao() {
        ...
        refreshCombo();
        enableFilds(true);
        BoletimEstatisticoRecebimento boletim = new BoletimEstatisticoRecebimentoTableModel(listBoletim).get(indexRow);
    ...
    private void removerProducao() {
        ...
        int result = new BoletimEstatisticoRecebimentoController().delete(boletim.getId());
        if (result == 1) {
    ...
    private void mostraDadosTable() {
        if (listBoletim != null) {
            form.getTbBoletimEstatitiscoRecebimento().setModel(new BoletimEstatisticoRecebimentoTableModel(listBoletim));
    ...
    private void refreshTableGeral() {
        ...
        listBoletim = new BoletimEstatisticoRecebimentoController().list(Date.valueOf(formato.format(de)),
    ...
    private void refreshTablePrestadorPeriodo() {
        ...
        java.util.Date de = (java.util.Date) form.getComboPesquisaDe().getDate();
        java.util.Date ate = (java.util.Date) form.getComboPesquisaAte().getDate();
        listBoletim = new BoletimEstatisticoRecebimentoController().listPrestadorPeriodo(
    ...
    private void refreshTablePrestadorProcePeriodo() {
        ...
        java.util.Date de = (java.util.Date) form.getComboPesquisaDe().getDate();
        java.util.Date ate = (java.util.Date) form.getComboPesquisaAte().getDate();
        listBoletim = new BoletimEstatisticoRecebimentoController().listPrestadorProcePeriodo(
                "%" + form.getComboPesquisaPrestador().getSelectedItem().toString() + "%",
    ...
    public void abrirRelPesquisarBoletimGeral() {
        ...
        InputStream inputStream = getClass().getResourceAsStream("/relMainBoletimGeral.jasper");

        Map parametros = new HashMap();
        parametros.put("de", Date.valueOf(formato.format(de)));
        parametros.put("ate", Date.valueOf(formato.format(ate)));
        try {
            ReportDao.openReport("Boletim Estatístico de recebimento de laudos - GERAL", inputStream, parametros,
    ...
    public void abrirRelPesquisarBoletimPrestadorNome() {
        ...
        InputStream inputStream = getClass().getResourceAsStream("/relMainBoletim.jasper");
        ...
        try {
            ReportDao.openReport("Boletim Estatístico de recebimento de laudos - MENSAL", inputStream, parametros,
                    CriaConexao.getConexao());
        } catch (SQLException exc) {
            exc.printStackTrace();
        } catch (JRException exc) {
    ...
    public void abrirRelPesquisarBoletimPresProcPeriodo() {
        ...
        InputStream inputStream = getClass().getResourceAsStream("/relMainBoletimPresProcPerio.jasper");
        ...
        try {
            ReportDao.openReport("Boletim Estatístico de recebimento de laudos - Prestador|Procedimento|Período", inputStream, parametros,
                    CriaConexao.getConexao());
        } catch (SQLException exc) {
            exc.printStackTrace();
        } catch (JRException exc) {
